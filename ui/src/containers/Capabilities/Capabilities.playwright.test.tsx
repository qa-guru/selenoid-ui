...
function renderCapabilities(_accessKey?: string) {
    return render(
...
async function selectPlaywrightChrome(user: any) {
...
describe("Capabilities Playwright Create Session", () => {
    ...
    it("shows accessKey in Playwright curl snippet from default accessKey", async () => {
        ...
        expect([...panel!.querySelectorAll(".ch-tok-cmd")].map(({ textContent }: any) => textContent)).toEqual([
            "curl",
            "localhost:3000",
        ]);
        expect([...panel!.querySelectorAll(".ch-tok-key")].map(({ textContent }: any) => textContent)).toContain(
            "accessKey"
        );
    });
