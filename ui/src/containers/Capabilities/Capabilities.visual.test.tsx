...
function expectColor(actual: any, expectedHex: any) {
...
describe("Capabilities visual contract (Driver + Remote hub + Browser caps panels)", () => {
    ...
    it("desktop columns are independent scrollports (config stack + terminal)", async () => {
        const fs = await import("node:fs/promises");
        const path = await import("node:path");
        const { fileURLToPath } = await import("node:url");
        const dir = path.dirname(fileURLToPath(import.meta.url));
    ...
    it("switches terminal paper with html.theme-light", async () => {
        const fs = await import("node:fs/promises");
        const path = await import("node:path");
        const { fileURLToPath } = await import("node:url");
        const dir = path.dirname(fileURLToPath(import.meta.url));
