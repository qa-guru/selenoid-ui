...
describe("Benchmarks", () => {
    ...
    it("fills Artifacts cost from Java cold pins, not go", () => {
        ...
        const log = table.querySelector('[data-preset="log"]');
        const video = table.querySelector('[data-preset="video"]');
        const harMeta = table.querySelector('[data-preset="har-meta"]');
        const harBodies = table.querySelector('[data-preset="har-bodies"]');
        const videoLog = table.querySelector('[data-preset="video+log"]');
        const videoLogHarMeta = table.querySelector('[data-preset="video+log+har-meta"]');
        const full = table.querySelector('[data-preset="video+log+har-bodies"]');
        expect(none).toHaveAttribute("data-status", "ok");
        expect(none).toHaveTextContent("2.729");
        expect(log).toHaveAttribute("data-status", "ok");
        expect(log).toHaveTextContent("3.939");
        expect(log).toHaveTextContent("73");
        expect(video).toHaveAttribute("data-status", "ok");
        expect(video).toHaveTextContent("5.259");
        expect(video).toHaveTextContent("44");
        expect(harMeta).toHaveAttribute("data-status", "ok");
        expect(harBodies).toHaveAttribute("data-status", "ok");
        expect(harBodies).toHaveTextContent("49");
        expect(videoLog).toHaveAttribute("data-status", "ok");
        expect(videoLogHarMeta).toHaveAttribute("data-status", "ok");
        expect(full).toHaveAttribute("data-status", "ok");
        expect(full).toHaveTextContent("5.296");
        expect(full).toHaveTextContent("166");
