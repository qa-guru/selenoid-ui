...
function headerPairs(headers: any) {
    if (!Array.isArray(headers)) {
        return [];
    }
    return headers
        .filter((h: any) => h && (h.name != null || h.value != null))
        .map((h: any) => ({ name: String(h.name || ""), value: String(h.value ?? "") }));
}

function HeaderKv({ title, headers }: any) {
...
function EntryDetail({ entry, tab, onTabChange }: any) {
...
const HarViewer = ({ session, browser = {}, sessionAlive = true, file: fileProp }: any) => {
    ...
    const [expandedIdx, setExpandedIdx] = useState<any>(null);
    ...
    const toggleRow = useCallback((idx: any) => {
        setExpandedIdx((prev: any) => {
            if (prev === idx) {
                return null;
            }
                                    ...
                                    return (
                                        <React.Fragment key={`${req.method || "GET"}-${req.url || ""}-${idx}`}>
                                            ...
                                            {open && (
                                                ...
                                                >
                                                    <td colSpan={6} onClick={(e: any) => e.stopPropagation()}>
                                                        <EntryDetail
                                                            entry={entry}
                                                            tab={detailTab}
