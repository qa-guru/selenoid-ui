...
WARNING: **Deprecated.** Актуальная документация qa-guru fork — в корневом link:../README.md[README.md]. Этот AsciiDoc оставлен для gh-pages / upstream-совместимости; образы ниже — `qaguru/*`, не `aerokube/*`.
...
of https://github.com/qa-guru/selenoid[Selenoid] on small go backend.
Source code available at https://github.com/qa-guru/selenoid-ui[GitHub].
...
$ docker run -d --name selenoid-ui -p 8080:8080 \
    ...
    --selenoid-uri http://${SELENOID_HOST}:4444 \
...
`$PWD` must contain the same `browsers.json` as the hub.
...
$ docker run -d --name selenoid-ui -p 8080:8080 \
    -v "$PWD:/etc/selenoid:ro" \
    qaguru/selenoid-ui:latest-release \
    --selenoid-uri http://${DOCKER_GATEWAY_ADDR}:4444 \
    --browsers-conf /etc/selenoid/browsers.json
----
...
fc479233071d        qaguru/selenoid        "/usr/bin/selenoid..."   10 minutes ago      Up 10 minutes       0.0.0.0:4444->4444/tcp                                  selenoid
----
+
- Just run linked UI:
+
...
$ docker run -d         \
    ...
    --browsers-conf /etc/selenoid/browsers.json <2>
----
<1> `selenoid` is name of *Selenoid* container
<2> should be the same as link name; mount the hub `browsers.json`
+
...
Canonical file: link:../docker-compose.yml[docker-compose.yml] (qaguru images + `browsers.json` volume for UI).
...
services:
  selenoid:
    image: "qaguru/selenoid:latest-release"
    network_mode: bridge
    environment:
  ...
  selenoid-ui:
    image: "qaguru/selenoid-ui:latest-release"
    network_mode: bridge
    links:
      - selenoid
    ports:
...
. Download Selenoid UI binary from https://github.com/qa-guru/selenoid-ui/releases[releases] page.

. Start it:

    $ ./selenoid-ui --selenoid-uri http://<selenoid-host-or-ip>:4444 --browsers-conf /path/to/browsers.json
...
Pass the same `browsers.json` as the hub via `-browsers-conf` so Playwright versions match.
...
You can view recorded videos on a separate tab. The UI loads videos via paginated `GET /video/?json` (10 per page) and does not embed the full video list into status/SSE.

=== Restricting Access

You can restrict access to Selenoid UI with a https://en.wikipedia.org/wiki/.htpasswd[htpasswd] file as follows:

. Create a file with user `test` and password `test-password`:
+
    $ htpasswd -bc /path/to/users.htpasswd test test-password

. Run Selenoid UI with `-users` flag pointing to this file:
+
...
----
  ...
  -browsers-conf string
    	browsers.json path with protocol metadata for the UI
  -listen string
...
For example:
```
$ ./selenoid-ui -allowed-origin "*" -period 500ms -browsers-conf ./browsers.json
```
