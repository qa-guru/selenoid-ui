...
for f in tokens header link input icon icon-btn lang-toggle button badge status-tile selenoid-metrics plaque-divider plaque-field plaque-field-seg plaque-field-seg-layout plaque-number panel sticky tab window-control connection-status vnc-window har-viewer code-highlight; do
