...
if [[ "$statik_size" -lt 100000 ]]; then
  echo "ERROR: statik/statik.go is only ${statik_size} bytes — ui-v2 frontend was not embedded" >&2
  echo "       Run: go generate ." >&2
  exit 1
fi
test -f ui-v2/index.html
...
mkdir -p licenses
echo "Selenoid UI v2 — vanilla JS, no npm runtime dependencies." > licenses/ui-licenses
